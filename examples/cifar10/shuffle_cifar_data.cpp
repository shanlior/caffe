...
void convert_dataset(const string& input_folder, const string& output_folder,
  ...
  for (int fileid = 0; fileid < kCIFARTrainBatches; ++fileid) {
    ...
    for (int itemid = 0; itemid < kCIFARBatchSize; ++itemid) {
      ...
      shuffle_vec.push_back(datum);
    }
  }
  std::random_shuffle(shuffle_vec.begin(), shuffle_vec.end());
  int id=0;
  for (vector<Datum>::iterator it = shuffle_vec.begin() ; it != shuffle_vec.end(); ++it) {
    int length = snprintf(str_buffer, kCIFARImageNBytes, "%05d", id);
    string out;
    CHECK(it->SerializeToString(&out));
  ...
  scoped_ptr<db::Transaction> txn_test(test_db->NewTransaction());
  // Open files
